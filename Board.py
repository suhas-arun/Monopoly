...
class Board:
    ...
    def drawPlayer(self, player):
        ...
        player_label.place(x=player_position_x, y=player_position_y, anchor="center")
    ...
    def updatePlayer(self, player):
        ...
        new_player_label.place(
            x=player_position_x, y=player_position_y, anchor="center"
        )
    ...
    def drawBoard(self):
        ...
        self.place_name_label = tkinter.Label(
            text="WELCOME TO\n MONOPOLY!",
            font=("Helvetica", 14),
            bg=self.default_colour,
        )
        self.place_name_label.place(x=327.5, y=320, anchor="center")

        self.info_label = tkinter.Label(
            text="Press Roll to start the game.",
            font=("Helvetica", 10),
            bg=self.default_colour,
        )
        ...
        for i in range(9):
            ...
            if place.type == "property":
                place_frame = tkinter.Frame(self.parent, width=54, height=69)
                ...
                )  # creates the rectangle with the colour of the property

                price_label = tkinter.Label(
                    place_frame, text="£{}".format(place.info[2]), font=("Helvetica", 8)
                )
                price_label.place(x=27, y=10, anchor="center")
                info_button = tkinter.Button(
                    place_frame,
                    text="\n".join(place.getName().upper().split()),
                    font=("Helvetica", 6),
                    relief="flat",
                )
                ...
                info_button.bind(
                    "<Button-1>",
                    lambda event, current_place=place: self.updateInfo(
                        event, current_place
                    ),
                )  # if you press the button of the place name, the place's info shows up in the info box.

            else:
                place_frame = tkinter.Frame(self.parent, width=54, height=79)
                place_frame.place(x=81 + 55 * i, y=0)
                self.canvas.create_rectangle(80 + 55 * i, 0, 80 + 55 * (i + 1), 80)
                ...
                if place.type in ["utility", "station"]:
                    ...
                    tkinter.Label(
                        ...
                        text="\n".join(place.getName().upper().split()),
                        font=("Helvetica", 6),
                    ).place(x=27, y=40, anchor="center")

        # left column
        for i in range(9):
            place = self.places[19 - i]
            if place.type == "property":
                place_frame = tkinter.Frame(self.parent, width=69, height=54)
                ...
                place_frame = tkinter.Frame(self.parent, width=79, height=54)
                place_frame.place(y=81 + 55 * i, x=0)
                self.canvas.create_rectangle(0, 80 + 55 * i, 80, 90 + 55 * (i + 1))
        ...
        # draws corner places
        corner1 = tkinter.Frame(self.parent, width=79, height=79).place(
            x=0, y=0
        )  # top left corner
        self.canvas.create_rectangle(0, 0, 80, 80)
        tkinter.Label(
            corner1, text="\n".join(self.places[20].getName().upper().split())
        ).place(x=40, y=40, anchor="center")

        corner2 = tkinter.Frame(self.parent, width=79, height=79).place(
            x=576, y=0
        )  # top right corner
        self.canvas.create_rectangle(575, 0, 655, 80)
        tkinter.Label(corner2, text="GO TO\nJAIL").place(x=615, y=40, anchor="center")

        # bottom left corner
        self.canvas.create_rectangle(0, 575, 80, 655, fill=self.default_colour)
        tkinter.Label(text="\n".join(self.places[10].getName().upper().split())).place(
            x=50, y=605, anchor="center"
        )
        # creates the "just visiting" thing
        self.canvas.create_line(20, 575, 20, 635)
        self.canvas.create_line(20, 635, 80, 635)
        tkinter.Label(text="J\nU\nS\nT", font=("Helvetica", 8)).place(
            x=10, y=610, anchor="center"
        )
        tkinter.Label(text="VISITING", font=("Helvetica", 7)).place(
            x=50, y=645, anchor="center"
        )

        corner4 = tkinter.Frame(self.parent, width=79, height=79).place(
            x=576, y=576
        )  # bottom right corner
        self.canvas.create_rectangle(575, 575, 655, 655)
        tkinter.Label(
            corner4, text=self.places[0].getName().upper(), font=("Helvetica", 32)
        ).place(x=615, y=615, anchor="center")
    ...
    def updateInfo(self, ignore, place, jail_info=None):
        ...
        if place.type == "property":
            ...
            if (
                self.currentPlayer
                and self.currentPlayer == place.owner
                and self.current_property.isMonopoly
                and self.current_property.noOfHouses < 5
            ):
        ...
        if place.type == "property":
            ...
            if (
                self.currentPlayer
                and place.owner == self.currentPlayer
                and self.current_property.noOfHouses > 0
            ):
        ...
        if place_type == "property":
            ...
            price, rent0, rent1, rent2, rent3, rent4, rent5, cost_of_house = (
                info[2],
                info[3],
                info[4],
                info[5],
            ...
            self.place_name_label.config(
                text=name.upper(), font=("Helvetica", 10), bg=self.default_colour
            )
            self.place_name_label.place(x=327.5, y=300, anchor="center")
            price = info[1]
            self.info_label.config(
        ...
        owner = (
            ...
            else "Owned by Player " + str(place.owner.number)
        )
    ...
    def displayPlayerInfo(self, player):
        ...
        player_number_label = tkinter.Label(
            player_frame,
            text="Player {}:".format(str(player.number)),
            bg=self.background_colour,
            fg=player.colour,
        ...
        balance_label = tkinter.Label(
            player_frame,
            text="Balance: £{}".format(player.balance),
            font=("Helvetica", 18),
            bg=self.background_colour,
        )
        ...
        for colour in self.properties.keys():
            ...
            if (
                colour == "yellow"
            ):  # yellow properties have a different background colour for visibility
                properties_info = tkinter.Label(
                    player_frame,
                    text="    ".join(prop.getName() for prop in props_of_colour),
                    fg=colour,
                    bg="grey",
                )
            else:
        ...
        stations_info = tkinter.Label(
            player_frame,
            text="    ".join(
                " ".join(station.getName().split()[:-1])
                for station in player.stations_owned
            ),
            bg=self.background_colour,
        )
        ...
        utilities_info = tkinter.Label(
            player_frame,
            text="    ".join(utility.getName() for utility in player.utilities_owned),
            bg=self.background_colour,
        )
    ...
    def displayPlayerOptions(self, currentPlayer, die1, die2, game):
        ...
        currentplayer_label = tkinter.Label(
            options_frame,
            text="Player {}'s turn".format(currentPlayer.number),
            bg=self.background_colour,
            font=("Helvetica", 16, "bold"),
            fg=currentPlayer.colour,
        )
        currentplayer_label.place(x=97, y=60, anchor="center")

        dice_total = die1 + die2
        self.dice_box = tkinter.Label(
            options_frame,
            text="Dice 1: {}\nDice2: {}\nTotal: {}".format(die1, die2, dice_total),
            bg="white",
            borderwidth=2,
            relief="raised",
            font=("Helvetica", 12),
        ...
        self.roll_button = tkinter.Button(
            options_frame,
            text="Roll",
            bg="white",
            font=("Helvetica", 16, "bold"),
            state="disabled",
            command=game.makeTurn,
        )
        self.roll_button.place(x=97, y=220, anchor="center")

        self.buy_button = tkinter.Button(
            options_frame,
            text="Buy",
            bg="white",
            font=("Helvetica", 16, "bold"),
        ...
        self.build_house = tkinter.Button(
            options_frame,
            text="Build House",
            bg="white",
            font=("Helvetica", 16, "bold"),
            state="disabled",
            command=game.buildHouse,
        )
        self.build_house.place(x=97, y=340, anchor="center")

        self.sell_house = tkinter.Button(
            options_frame,
            text="Sell House",
            bg="white",
            font=("Helvetica", 16, "bold"),
            state="disabled",
            command=game.sellHouse,
        )
        self.sell_house.place(x=97, y=400, anchor="center")

        self.sell_property = tkinter.Button(
            options_frame,
            text="Sell Property",
            bg="white",
            font=("Helvetica", 16, "bold"),
            state="disabled",
            command=game.sellProperty,
        )
    ...
    def flashDiceBox(self):
        ...
        self.parent.after(
            1000, lambda: self.dice_box.config(bd=2, font=("Helvetica", 12,))
        )
